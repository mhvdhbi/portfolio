...
function CaseStudy({ p }: { p: Project }) {
  return (
    <article className="mt-14">
      ...
      {/* Numbers first — a prospect skims, and each of these can be verified. */}
      <Reveal delay={0.1}>
      ...
      {p.shots.gallery && (
        <div className="mt-16 grid gap-6 sm:grid-cols-2">
          {p.shots.gallery.map((g, i) => (
            <Reveal key={g.src} delay={(i % 2) * 0.07}>
      ...
      {/* Stated plainly. Anyone who rings the restaurant finds out either way,
          and being straight about it reads as confidence. */}
      {p.status === 'concept' && (
        <Reveal>
...
export function Work() {
  ...
  return (
    <section id="travaux" className="relative scroll-mt-24 border-t border-line py-20 sm:py-28">
      <div className="container-y">
              ...
              <Reveal key={p.slug} delay={(i % 2) * 0.07}>
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    <p className="mt-1.5 font-mono text-[11px] text-text-3">{p.sector}</p>
                    <p className="mt-3 text-[14px] leading-relaxed text-text-2">{p.summary}</p>
                  </div>
                </a>
              </Reveal>
            ))}
        ...
        <Reveal delay={0.08}>
          <a
            ...
            className="group mt-6 flex flex-col items-center justify-center rounded-2xl border border-dashed border-line-2 px-8 py-16 text-center transition-colors hover:border-cyan"
          >
            <span className="flex h-12 w-12 items-center justify-center rounded-full border border-line-2 text-text-3 transition-colors group-hover:border-cyan group-hover:text-cyan">
              <svg viewBox="0 0 24 24" className="h-5 w-5" aria-hidden="true">
                <path d="M12 5v14M5 12h14" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
              </svg>
            </span>
            <span className="display-md mt-5">Votre projet ici</span>
            <span className="mt-2 max-w-[44ch] text-[14px] text-text-2">
              Parlez-moi de votre activité et je vous montre à quoi pourrait
